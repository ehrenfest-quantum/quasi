...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        {
            name: "simulate_noisy",
            description: "Estimate circuit fidelity under realistic noise for an Ehrenfest CBOR program. Compiles via Afana to OpenQASM 3.0, then runs noise-aware simulation using MQT DDSIM (ideal vs noisy, TVD fidelity). Falls back to heuristic gate-fidelity estimate if DDSIM is not installed. Use this to check whether a circuit has a reasonable chance of producing useful results on a given backend before submitting to hardware.",
            inputSchema: {
                type: "object",
                properties: {
                    cbor_path: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === "simulate_noisy") {
            const { cbor_path, backend, shots, sq_err, tq_err } = args;
