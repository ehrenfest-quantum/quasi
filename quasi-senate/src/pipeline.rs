...
use crate::ledger::ProposalOutcome;
...
/// Rollback lever: `SENATE_DIRECT_ISSUES=1` restores the pre-gate behaviour of
/// creating GitHub issues directly, bypassing the quasi-board proposal queue.
/// Default (unset) is the proposal path required by CLAUDE.md invariant 3.
pub fn direct_issues_enabled() -> bool {
...
/// returns 0 (no issue exists until a human accepts the proposal). With
/// `SENATE_DIRECT_ISSUES=1`, opens the GitHub issue directly instead and
...
pub async fn run_draft_pipeline(ctx: &mut AppContext) -> Result<u32> {
    ...
    for retry in 0..4u32 {
        ...
        if verdict.verdict == Verdict::Approve {
            ...
            // How the approved draft is published: queued as a quasi-board
            // proposal (default), or opened as a GitHub issue directly when
            // the SENATE_DIRECT_ISSUES=1 rollback lever is set.
            enum Publication {
                Issue(crate::types::Issue),
                Proposal(String),
            }

            let publication = if direct_issues_enabled() {
                warn!(
                    "pipeline: SENATE_DIRECT_ISSUES=1 — bypassing the quasi-board \
                     proposal gate; creating GitHub issue directly"
                );
                ...
                match crate::ledger::submit_proposal(&draft, &verdict.reasoning).await {
                    ProposalOutcome::Queued(prop_id) => {
                        info!(
                            "pipeline: proposal {prop_id} queued on quasi-board for '{}' — awaits human acceptance",
                            draft.title
                        );
                        Publication::Proposal(prop_id)
                    }
                    ProposalOutcome::Rejected(msg) => {
                        warn!("pipeline: board rejected proposal (400): {msg}");
                        return Err(anyhow!(
                            "quasi-board rejected draft '{}' as trivial or malformed: {msg}",
                            draft.title
                        ));
                    }
                    ProposalOutcome::Duplicate(msg) => {
                        warn!("pipeline: board flagged near-duplicate proposal (409): {msg}");
                        return Err(anyhow!(
                            "quasi-board flagged draft '{}' as a near-duplicate: {msg}",
                            draft.title
                        ));
                    }
                    ProposalOutcome::L0Cap(msg) => {
                        warn!("pipeline: board L0 proposal cap reached (429): {msg}");
                        return Err(anyhow!("quasi-board L0 proposal cap reached: {msg}"));
                    }
                    ProposalOutcome::Failed(msg) => {
                        warn!("pipeline: proposal submission failed: {msg}");
                        return Err(anyhow!(
                            "proposal submission failed for '{}': {msg}",
                            draft.title
                        ));
                    }
                }
            ...
            let (issue_number, downstream_verdict, ledger_event, ledger_url) = match &publication {
                Publication::Issue(issue) => (
                    Some(issue.number),
                    "approved",
                    "issue_generated",
                ...
                Publication::Proposal(prop_id) => {
                    (None, "proposed", "proposal_queued", prop_id.clone())
                }
            };

            // Write A.2 and A.3 telemetry — approved path, with issue_number
            // known only on the direct-issue path.
            record_telemetry(
                &ctx.db,
                &TelemetryEntry {
                    timestamp: Utc::now(),
                    ...
                    json_parse_ok: Some(true),
                    downstream_verdict: Some(downstream_verdict.to_string()),
            ...
            if let Some(bot) = &ctx.matrix {
                let (plain, html) = match &publication {
                    Publication::Issue(issue) => (
                        format!(
                            "✅ Issue #{} opened: {} — {}",
                    ...
                    Publication::Proposal(prop_id) => (
                        format!(
                            "📋 Proposal {prop_id} queued: {} — awaits human acceptance on quasi-board",
                            draft.title
                        ),
                        format!(
                            "<p>📋 <b>Proposal {prop_id} queued:</b> {title} — awaits human acceptance on quasi-board</p>",
                            prop_id = prop_id,
                            title = draft.title,
                        ),
                    ),
                ...
                match bot.join_room("#senate-council:paulsboutique.hal-contract.org").await {
                    Ok(room_id) => {
                        if let Err(err) = bot.send_message(&room_id, &plain, &html).await {
                            warn!("pipeline: failed to post approval to Matrix: {err}");
                        }
                    }
                    Err(err) => {
            ...
            if let Some(fedi) = &ctx.fedi {
                let status = match &publication {
                    Publication::Issue(issue) => format!(
                        "✅ QUASI Senate Loop opened issue #{}: {} {}",
                        issue.number, draft.title, issue.html_url
                    ),
                    Publication::Proposal(prop_id) => format!(
                        "📋 QUASI Senate Loop queued proposal {prop_id} for human acceptance: {}",
                        draft.title
                    ),
                };
            ...
            return Ok(match publication {
                Publication::Issue(issue) => issue.number,
                // No issue exists yet — the proposal awaits human acceptance.
                Publication::Proposal(_) => 0,
            });
        }
...
pub async fn run_cycle(ctx: &mut AppContext) -> Result<()> {
    ...
    if issue_number == 0 {
        info!("pipeline: proposal queued on quasi-board — no issue to solve yet; skipping B-track");
        return Ok(());
    }
...
pub async fn run_batch(ctx: &mut AppContext, count: u32) -> Result<()> {
    ...
    for i in 0..count {
        ...
        match run_draft_pipeline(ctx).await {
            // 0 = proposal queued on quasi-board; no issue exists to solve.
            Ok(0) => {}
