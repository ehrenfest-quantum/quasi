...
/// The ROTATION slice must contain exactly 66 models (see config.rs comment for full breakdown).
#[test]
fn test_rotation_count() {
    assert_eq!(ROTATION.len(), 66, "Expected 66 models in ROTATION, got {}", ROTATION.len());
}
